...
namespace GeminiClient
{
    ...
    public class GeminiResponse
    {
        ...
        public GeminiResponse(string responseHeader, string responseBody = "")
        {
            ...
            Meta = responseHeader.Substring(split, responseHeader.Length - split);
    ...
    public class GeminiClient : System.IDisposable
    {
        ...
        public GeminiResponse Fetch(Uri uri, X509Certificate certificate, bool verbose)
        {
            var host = uri.Host;
            var port = uri.Port;
            ...
            using (var stream = new SslStream(client.GetStream(), false, callback, null))
            {
                ...
                if (verbose) Console.WriteLine("Debug: Sending request: {0}", uri.AbsoluteUri + "\r\n");
                ...
                stream.Read(buffer);
                if (verbose) Console.WriteLine("Debug: Remote server returned header: {0}", System.Text.Encoding.UTF8.GetString(buffer));
                ...
                if (response.StatusType == GeminiStatusType.Success)
                {
                    ...
                    {
                        stream.CopyTo(ms);
                        response.ResponseBody = System.Text.Encoding.UTF8.GetString(ms.ToArray());
                    }
                }
                return response;
            }
